...
export function TrustPanel(){return <section className="trust"><h2><ShieldCheck/> שקיפות לפני הכול</h2>{[[LockKeyhole,'כל המלצה נשארת','אחרי הפרסום אי אפשר לערוך או למחוק. גם כשהתוצאה לא כצפוי.'],[BarChart3,'כל התוצאות נספרות','אחוז פגיעה, מספר המלצות שנסגרו ויחס ממוצע — בתמונה אחת.'],[ShieldCheck,'היסטוריה שאפשר לבדוק','התוצאות, היחסים והתיקונים פתוחים לעיון של כולם.']].map(([Icon,title,body])=>{const I=Icon as typeof ShieldCheck;return <div className="trust-row" key={String(title)}><I size={22}/><div><strong>{String(title)}</strong><p>{String(body)}</p></div></div>;})}<Link href="/how-it-works">כך מחושב הדירוג <ArrowLeft size={15}/></Link></section>;}

export function Streak({count}:{count:number}){return count>3?<span className="streak" title="רצף הצלחות בהמלצות שנסגרו, לפי סדר הפרסום. ביטולים אינם נספרים.">🔥 {count} ברצף</span>:null;}
